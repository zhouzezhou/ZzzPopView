...
//    }
    ...
    // 2018年04月17日 Zzz 添加shouldChangeCharactersInRange代理方法的
    if ([inputView isKindOfClass:[UITextField class]]) {
        UITextField *tmp = (UITextField *)inputView;
        //            [tmp deleteBackward];
...
//            }
            ...
            if (tmp.delegate && [tmp.delegate respondsToSelector:@selector(textField:shouldChangeCharactersInRange:replacementString:)]) {
                ...
                BOOL ret = [tmp.delegate textField:tmp shouldChangeCharactersInRange:range replacementString:btn.titleLabel.text];
                if (ret) {
                    [tmp insertText:btn.titleLabel.text];
                }
            }else{
                [tmp insertText:btn.titleLabel.text];
                //                [tmp insertText:@"•"];
